...
class DebugNode(LifecycleNode):
    ...
    def draw_box(
        ...
        """Draw a bounding box on the image with a bigger, more visible label."""
        ...
        for i in range(4):
            ...
            cv2.line(cv_image, pt1, pt2, color, box_thickness)
        ...
        bg_color = color               # fondo del mismo color de la caja
        ...
        # Si queda muy arriba, lo ponemos abajo de la caja
        if text_y - text_h - baseline < 0:
            text_y = min_pt[1] + text_h + 10

        # Rectángulo de fondo para que resalte
        cv2.rectangle(
            cv_image,
            (text_x, text_y - text_h - baseline - 6),
            (text_x + text_w + 10, text_y + 6),
            bg_color,
            -1,
        )
    ...
    def draw_keypoints(self, cv_image: np.ndarray, detection: Detection) -> np.ndarray:
        ...
        for kp in keypoints_msg.data:
            ...
            cv2.putText(
                cv_image,
                str(kp.id),
                (int(kp.point.x) + 4, int(kp.point.y) - 4),
